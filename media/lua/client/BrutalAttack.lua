	...
	----------------------------------------------------------
	-- Clombat mode: true if wearing gloves OR debug
	----------------------------------------------------------
	function BrutalAttack.IsClombatMode(player)
	    if not player then return false end
	
	    -- Debug always counts as "on"
	    if SpookyUtils and SpookyUtils.isDebug and SpookyUtils.isDebug(player) then
			player:setVariable("isClombat", true)
	        return true
	    end
	
	    -- Gloves condition
	    if SpookyUtils and SpookyUtils.hasTagInSlot then
	        if SpookyUtils.hasTagInSlot(player, "Hands", "ClombatGloves") then
			player:setVariable("isClombat", true)		
	            return true
	        end
	    end
		player:setVariable("isClombat", false)
	    return false
	end
	...
	-----------------------------------------------------------
	-- BrutalAttack – Melee / Unarmed Combat Resolution
	--
	-- Intent:
	-- This module is a weapon-agnostic combat engine that
	-- reproduces and extends Project Zomboid’s melee logic
	-- in Lua for advanced control and mod interoperability.
	--
	-- It acts as the *authoritative local combat resolver*:
	-- - Target discovery
	-- - Damage calculation
	-- - Hit ordering
	-- - Combat speed calculation
	--
	-- Key Responsibilities:
	-- - Calculate melee damage and delta
	-- - Determine valid hit targets and ordering
	-- - Apply endurance, injury, perk, and moodle modifiers
	-- - Support unarmed ("Clombat") and weapon combat
	--
	-- Multiplayer Notes:
	-- - NOT fully MP-safe by itself
	-- - Requires BHPatch for networked zombie damage
	--
	-- Explicit Non-Goals:
	-- - No animation control
	-- - No stance replication
	-- - No UI logic
	
	-- Scope Boundary:
	-- - BrutalAttack never handles ballistics or penetration
	-- - Melee logic is intentionally isolated from
	--   Advanced Trajectory to avoid cross-system coupling
	...
	local setup = function()
		...
		BrutalAttack.ClombatHands   = InventoryItemFactory.CreateItem("Base.Fisticuffs_Clombat")
